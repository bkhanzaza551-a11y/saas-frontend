...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    metaDesc.content = config.heroSubtitle || `Book premium salon services at ${salon.name}`;
    return () => { document.title = "ReSpark"; };
  }, [salon.name, config.heroSubtitle]);
                    ...
                    <div className="sf-service-info">
                      ...
                      <div className="sf-service-footer">
                        <div className="sf-service-price-row">
                          <span className="sf-service-price">{formatPrice(price, currency)}</span>
                          {hasSale && (
                            <span className="sf-service-original-price">{formatPrice(originalPrice, currency)}</span>
                          )}
                        </div>
                        {staff.length > 0 && (
                          <div className="sf-service-staff">
                            <div className="sf-service-staff-avatars">
                              {staff.slice(0, 3).map((s, i) => (
                                <img
                                  key={i}
                                  src={s.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(s.name || "")}&background=c8a97e&color=fff&size=100`}
                                  alt={s.name}
                      ...
                      <Link to={`/site/${salon.slug}/service/${service.id}`} className="sf-btn sf-btn-primary sf-service-book-btn">
                        Book Now
                      </Link>
                    </div>
                  </div>
