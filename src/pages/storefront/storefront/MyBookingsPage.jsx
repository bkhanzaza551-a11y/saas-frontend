...
function BookingCard({ booking, currency, onCancel, onReschedule }) {
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          {booking.total != null && (
            <span style={{ fontSize: "0.95rem", fontWeight: 700 }}>
              {currency} {Number(booking.total).toFixed(2)}
            </span>
          )}
        </div>
...
export default function MyBookingsPage() {
  ...
  const handleCancel = (booking) => {
    if (!window.confirm(`Cancel booking ${booking.orderNumber}? This action cannot be undone.`)) return;
    setLoading(true);
    api
      .patch(`/public/salon/${slug}/my-bookings/${booking.orderNumber}/cancel`, { phone: phone.trim() })
      .then(() => {
        setBookings((prev) =>
  ...
  const handleReschedule = (booking) => {
    alert(`Reschedule for ${booking.orderNumber} is not yet available. Please contact the salon directly.`);
  };
