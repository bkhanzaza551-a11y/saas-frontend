...
export default function OwnerProductsRequirementPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.patch(`/owner/product-requirements/${editingId}`, payload);
        setStatus({ error: "", success: "Product requirement updated." });
      } else {
        await api.post("/owner/product-requirements", payload);
    ...
    <div className="page-shell">
      <div className="hero-card" style={{ padding: 24, marginBottom: 20 }}>
        <div className="item-head">
          <div>
            ...
            <p style={{ marginBottom: 0 }}>Request salon inventory, equipment, or supplies from administration.</p>
          </div>
          <button className="btn btn-primary" onClick={() => { setForm(emptyForm); setEditingId(null); setIsModalOpen(true); }}>
      ...
      <div className="panel-card" style={{ padding: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ margin: 0 }}>My Requisitions</h3>
        ...
        {filtered.length === 0 ? <EmptyState title="No requirements found" /> : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {filtered.map((r) => {
              ...
              const branchName = branches.find(b => b.id === r.branchId)?.name || "All Branches";
                ...
                <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", background: "#f8fafc", borderRadius: 12, border: "1px solid #e2e8f0" }}>
                  <div style={{ display: "flex", gap: 24, alignItems: "center", flex: 1 }}>
                    ...
                    <div style={{ minWidth: 180 }}>
                      ...
                      <div style={{ fontSize: "0.75rem", color: "#64748b", marginTop: 4, display: "flex", alignItems: "center", gap: 4 }}><Building2 size={12} /> {branchName}</div>
                    </div>
                    <div style={{ minWidth: 100 }}>
                      <div style={{ fontSize: "0.85rem", color: "#475569" }}>{r.category || "-"}</div>
                      <div style={{ fontSize: "0.78rem", color: "#94a3b8" }}>Qty: {r.quantity || 1}</div>
                    </div>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexDirection: "column", alignItems: "flex-end" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                      <button type="button" onClick={() => startEdit(r)} style={{ padding: "4px 10px", fontSize: "0.75rem", borderRadius: 6, border: "1px solid #e2e8f0", background: "white", cursor: "pointer" }}>Edit</button>
                      <button type="button" onClick={() => deleteReq(r.id)} style={{ padding: "4px 10px", fontSize: "0.75rem", borderRadius: 6, border: "1px solid #fecaca", background: "#fef2f2", color: "#ef4444", cursor: "pointer" }}>Delete</button>
                    </div>
                  </div>
                </div>
              );
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
          <div style={{ background: "white", width: "100%", maxWidth: 540, borderRadius: 16, padding: 24, boxShadow: "0 10px 25px rgba(0,0,0,0.15)", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, borderBottom: "1px solid #eee", paddingBottom: 12 }}>
              ...
              <button onClick={() => { setIsModalOpen(false); setForm(emptyForm); setEditingId(null); }} style={{ border: "none", background: "none", cursor: "pointer", fontSize: 18, color: "#94a3b8" }}>✕</button>
            </div>
            ...
            <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Shampoo, Trimmer, Towels"
                  required
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  ...
                  <input
                    value={form.category}
                    placeholder="e.g. Haircare, Tools, Furniture"
                ...
                <div>
                  ...
                  <input
                    value={form.vendor}
                    placeholder="e.g. L'Oreal, Dyson, Local Supplier"
              ...
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end", marginTop: 12, borderTop: "1px solid #eee", paddingTop: 16 }}>
                <button type="button" onClick={() => { setIsModalOpen(false); setForm(emptyForm); setEditingId(null); }} className="btn btn-secondary">Cancel</button>
