...
const statusTheme = {
  NOT_CHECKED_IN: { label: "NC", bg: "#f1f5f9", color: "#475569" },
  PRESENT: { label: "P", bg: "#dcfce7", color: "#166534" },
  LATE: { label: "L", bg: "#fef3c7", color: "#92400e" },
  HALF_DAY: { label: "H", bg: "#fde68a", color: "#854d0e" },
  CHECKED_OUT: { label: "CO", bg: "#dbeafe", color: "#1e40af" },
  ABSENT: { label: "A", bg: "#fee2e2", color: "#b91c1c" },
  LEAVE: { label: "LV", bg: "#e0e7ff", color: "#3730a3" },
  OFF: { label: "OFF", bg: "#e2e8f0", color: "#475569" }
};

const attendanceMetricKeys = ["present", "late", "halfDay", "absent", "leave"];
...
export default function PayrollPage() {
  ...
  const [attendanceSummary, setAttendanceSummary] = useState({ totalStaff: 0, presentToday: 0, absentToday: 0, lateStaff: 0, currentlyWorking: 0, completedShift: 0, onLeave: 0, attendancePercentage: 0 });
  ...
  const attendanceCalendarRows = useMemo(() => {
    ...
    (attendanceCalendar.rows || []).forEach((row) => {
      ...
      if (row.status === "PRESENT" || row.status === "CHECKED_OUT") staffRow.totals.present += 1;
        ...
        <div className="panel-card">
          ...
          <div className="list-stack" style={{ gap: 12, marginTop: 16, overflowY: "auto", maxHeight: "55vh" }}>
            {(attendanceReport.rows || []).slice(0, 12).map((row, index) => {
              const statusColors = { NOT_CHECKED_IN: { bg: "#f1f5f9", color: "#475569" }, PRESENT: { bg: "#dcfce7", color: "#166534" }, LATE: { bg: "#fef9c3", color: "#854d0e" }, HALF_DAY: { bg: "#ffedd5", color: "#9a3412" }, CHECKED_OUT: { bg: "#dbeafe", color: "#1e40af" }, ABSENT: { bg: "#fee2e2", color: "#991b1b" }, LEAVE: { bg: "#ede9fe", color: "#5b21b6" } };
