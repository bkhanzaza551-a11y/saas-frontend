        ...
        <div className={`pos-main ${mobileTab === "catalog" ? "mobile-hidden" : ""}`}>
          ...
          <div className="pos-footer-bar">
            ...
            {form.items.some(i => i.itemType === "SERVICE") && (
              <button type="button" className="pos-btn-create" disabled={submitting || !!activeServiceInvoice || runningServices.some(rs => rs.form.customerId === form.customerId)} title={activeServiceInvoice ? "Customer already has an active service. Complete it first." : (runningServices.some(rs => rs.form.customerId === form.customerId) ? "Customer already has a running service. Complete it first." : "")} onClick={() => !submitting && submitInvoice("start")}>Start</button>
            )}
