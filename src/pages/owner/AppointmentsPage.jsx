...
export default function AppointmentsPage() {
  ...
  const handleAddGuest = async (e) => {
    ...
    try {
      ...
      setGuestSearchInput(newCustomer.name);
      setForm(c => ({ ...c, customerId: newCustomer.id }));
      if (newCustomer.gender) {
        setServiceGenderFilter(newCustomer.gender.toUpperCase());
      }
            ...
            <form onSubmit={handleFormSubmit} style={{ display: "flex", flexDirection: "column", flexGrow: 1, overflow: "hidden" }}>
              <div className="sp-body" ref={spBodyRef}>
                ...
                <div className="sp-card">
                  <h4 className="sp-card-title" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                    <span>1. Customer Details {editMode && form.customerId && <button className="add-link" type="button" onClick={() => navigate(`/admin/customers/${form.customerId}`)}>Guest History</button>}</span>
                    <PermissionButton type="button" className="add-link" module="customers" action="create" onClick={() => {
                      ...
                      }} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4, fontWeight: 600, fontSize: "12px", padding: 0 }}>
                      <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" /></svg>
                      Add Customer
                    </PermissionButton>
                  </h4>
      ...
      {hoveredAppt && (
        <div style={tooltipStyle}>
          ...
          <div style={{ fontSize: "0.78rem", color: "#1e293b", fontWeight: "500", marginBottom: "8px" }}>
            Customer: {hoveredAppt.appt.customer?.name || "Walk-in"}{hoveredAppt.appt.customer?.phone ? `, ${hoveredAppt.appt.customer.phone}` : ""}
          </div>
          <div>
