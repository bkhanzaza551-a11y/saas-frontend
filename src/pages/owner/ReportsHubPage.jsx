...
const COLUMNS = {
  sales_summary: ["SR. NO.", "DATE", "TIME", "INVOICE NO", "CUSTOMER NAME", "CUSTOMER NUMBER", "ITEMS", "GROSS AMOUNT", "DISCOUNT", "TAX", "NET TOTAL", "PAID AMOUNT", "DUE AMOUNT", "PAYMENT MODE"],
  product_sales: ["SR. NO.", "DATE", "TIME", "CUSTOMER NAME", "CUSTOMER NUMBER", "INVOICE NO", "PRODUCT", "CATEGORY", "QTY", "UNIT PRICE", "COMPLIMENTARY", "REDEMPTION AMOUNT", "TAX", "SUBTOTAL", "TOTAL", "PAYMENT MODE"],
  service_sales: ["SR. NO.", "Date", "Time", "CUSTOMER NAME", "CUSTOMER NUMBER", "Staff", "Invoice No", "Service", "Category", "Duration", "Qty", "Unit Price", "Discount", "Complimentary", "Redemption Amount", "Redemption Sources", "Tax", "Subtotal", "Total"],
  staff_performance: ["SR. NO.", "STAFF", "TOTAL SERVICES DONE", "UNIQUE CUSTOMER COUNT", "SERVICES WITHOUT TAX", "PRODUCTS WITHOUT TAX", "AVERAGE RETAIL SALE", "PACKAGES REVENUE", "PACKAGES SOLD", "MEMBERSHIP SOLD", "MEMBERSHIP REVENUE", "GIFTCARD REVENUE", "DISCOUNT", "TOTAL WITHOUT TAX", "COMPLIMENTARY AMOUNT", "REDEMPTION AMOUNT", "AVERAGE BILL VALUE"],
  monthly_sale: ["SR. NO.", "DATE", "INVOICE", "CUSTOMER NAME", "CUSTOMER NUMBER", "STAFF", "SUBTOTAL", "DISCOUNT", "INCLUSIVE TAX", "EXCLUSIVE TAX", "TOTAL", "PAYMENT MODE", "REDEMPTION AMOUNT", "BALANCE CLEARED", "ACTUAL TOTAL"],
  ...
  packages: ["SR. NO.", "INVOICE NO.", "PURCHASE DATE", "BUSINESS DATE", "EXPIRY DATE", "CUSTOMER NAME", "CUSTOMER NUMBER", "STAFF", "PACKAGE NAME", "SUBTOTAL", "TAX", "TOTAL", "PAYMENT MODE", "PURCHASE SOURCE"],
...
const REPORT_FILTERS = {
  ...
  customers: [
    { key: "customerId", label: "Customer", type: "select", endpoint: "/owner/customers", optionLabel: "name", defaultLabel: "All" }
  ],
  staff_performance: [
    { key: "stylistId", label: "Stylist", type: "select", endpoint: "/owner/staff-users", optionLabel: "name", defaultLabel: "All" }
  ],
...
const normalizeRowForReport = (reportKey, row, index) => {
  ...
  if (reportKey === "packages") {
    if (row["CUSTOMER NAME"] !== undefined || row["INVOICE NO."] !== undefined) return row;
    return {
      Date: prettifyDate(row.createdAt),
      Customer: row.customer?.name || "-",
...
function GuestCollectionChart({ rows }) {
  ...
  const dataRows = (rows || []).filter((r) => r && r["CUSTOMER NAME"] && r["CUSTOMER NAME"] !== "TOTAL" && (Number(r["TOTAL"]) || 0) > 0);

  const chartData = useMemo(() => {
    return dataRows
      .map((r) => ({
        name: r["CUSTOMER NAME"] || r["CUSTOMER NAME"] || "Customer",
        phone: r["CUSTOMER NUMBER"] || r["CUSTOMER NUMBER"] || "",
  ...
  if (chartData.length === 0) {
    return (
      <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 8, padding: 20, marginTop: 16, textAlign: "center", color: "#94a3b8", fontSize: 14 }}>
        No data available for the chart. Run the report to see customer-wise sales visualization.
      </div>
    );
  }
...
function ReportChart({ reportKey, rows }) {
  ...
  const data = useMemo(() => {
    ...
    if (reportKey === "customers") {
      return filtered
        .filter((r) => pick(r, "CUSTOMER NAME", "CUSTOMER NAME") && pick(r, "CUSTOMER NAME", "CUSTOMER NAME") !== "TOTAL")
        .map((r) => ({ name: pick(r, "CUSTOMER NAME", "CUSTOMER NAME") || "Customer", value: Number(pick(r, "TOTAL", "Total Spend", "Total")) || 0 }))
        .filter((d) => d.value > 0)
        .sort((a, b) => b.value - a.value)
        .slice(0, 10);
    }
    ...
    if (reportKey === "tip_report") {
      const grouped = {};
      filtered.forEach((r) => {
        const name = pick(r, "CUSTOMER NAME", "CUSTOMER NAME") || "-";
    ...
    if (reportKey === "complimentary" || reportKey === "cancelled_invoices") {
      return filtered
        .slice(0, 10)
        .map((r) => ({ name: pick(r, "CUSTOMER NAME", "CUSTOMER NAME", "Customer") || "-", value: Number(pick(r, "TOTAL", "Total", "VALUE", "Value")) || 0 }));
    }
    return filtered
      .slice(0, 10)
      .map((r, idx) => {
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem", color: "#334155", borderTop: "1px dashed #e2e8f0", paddingTop: "4px" }}>
              <span>Customer Purchased Services & Products</span>
